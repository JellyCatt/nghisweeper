...
global_type::ReturnStruct UIManager::FeedCommand(const std::vector<std::string>& command) {
  ...
  if (current_ui_->GetCurrentState() == UI::UIState::END) {
    return InitNextUI(current_ui_->GetNextSession());
  }
  return ret;
}

global_type::ReturnStruct UIManager::InitNextUI(UISession next_session) {
  global_type::ReturnStruct ret;
  switch (next_session) {
  case UISession::MAIN_MENU:
    ret.state_ = global_type::ReturnState::OK;
    current_ui_ = std::make_shared<ui::MainMenuUI>(renderer_, logger_);
    break;
  case UISession::DIFFICULTY_SELECT:
    ret.state_ = global_type::ReturnState::OK;
    current_ui_ = std::make_shared<ui::DifficultyUI>(renderer_, logger_);
    break;
  case UISession::GAME_PLAY:
    ret.state_ = global_type::ReturnState::OK;
    current_ui_ = std::make_shared<ui::GameplayUI>(renderer_, logger_);
    break;
